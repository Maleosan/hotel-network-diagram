...
async function prepareGuestPicker(){
    ...
    // Published data is intentionally public in Firestore rules. Do not block
    // this flow on Firebase Anonymous Auth, which may be disabled or filtered
    // by the deployment even though public reads are available.
    await Promise.resolve();
    if(generation!==guestLoadGeneration)return;
    guestSession=true;guestLoginRequested=false;currentUser=null;bridge.setStorageUser(null);setGuestProgress(1,"Guest session ready. Loading Published Diagrams…");
...
async function initializeFirebase(){
    ...
    logoutButton.addEventListener("click",async()=>{
        ...
        if(guestSession){guestSession=false;guestLoadGeneration++;logoutButton.textContent="Logout";bridge.setGuestMode(false);showLogin("Guest session ended.");logoutButton.disabled=false;return;}
